...
function drawVector(start, end, c) {
  stroke(c);
  strokeWeight(4);
...
function setup() {
  ...
  A = createVector(150, -100, 80);
  B = createVector(80, 120, -60);
...
function draw() {
  ...
  drawVector(createVector(0, 0, 0), A, color(255, 0, 0));
  drawVector(A, R, color(0, 0, 255));
  drawVector(createVector(0, 0, 0), R, color(0, 255, 0));
...
function keyPressed() {
  if (key === '1') {
    animateCameraTo({ x: targetCoordinates[0], y: targetCoordinates[1], z: 800 }, { x: 0, y: 0, z: 0 }, 1200);
  }
}
